    ...
    <div class='card-body'>
        <form action="editarProducto/{$producto->id}" method="POST" class="my-4" enctype="multipart/form-data">
            <table class='table'>
                ...
                <tbody>
                    <tr>
                    <td><input value="{$producto->productoNombre}" placeholder="Nombre nuevo" name="nombre" type="text" class="form-control" required></td>
                    <td><input value="{$producto->marca}" placeholder="Nombre marca nuevo" name="marca" type="text" class="form-control" required></td>
                    <td><input  value="{$producto->precio}" placeholder="Precio nuevo" name="precio" type="number" class="form-control" required></td>
                    <td><select name="idCategoria" class="form-select" required>
                            {foreach from=$listadoCategorias item=$item}
                                {if $item->nombre == $producto->categoriaNombre}
                                    <option selected="{$item->id}" value="{$item->id}">{$item->nombre}</option>
                                {else}
